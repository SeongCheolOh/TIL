-- 10장 조인 11장 서브쿼리
select * from emp, dept; -- 두개 테이블 X 곱하기
select * from emp where deptno=20; -- 18개 레코드
select * from dept; -- 4개 레코드
select 18*4 from dual;

select * from emp, dept where emp.deptno = dept.deptno; -- equal join

-- 김사랑의 부서명과 지역명은? 김사랑 인사부 인천
select ename, dname, loc from emp, dept where emp.ename = '김사랑' and emp.deptno = dept.deptno ;
-- 지역인 인천인 사람의 사원번호와 입사일을 출력하기
select emp.deptno, hiredate, loc from emp, dept where emp.deptno = dept.deptno and dept.loc='인천';
-- 인사부, 경리부인 사람의 사원번호, 이름, dname출력하기
select emp.deptno, emp.ename, dept.dname from emp, dept 
      where emp.deptno=dept.deptno and dept.dname in ('인사부', '경리부')
...
select * from emp; 
-- 자신의 테이블을 2번이상 써서 조인하는 것-셀프조인 
select * from emp e1, emp e2 where e1.mgr = e2.empno;
select e1.ename || '씨 매니저는 ' || e2.ename  사원의매니저 from emp e1, emp e2 where e1.mgr = e2.empno;

select * from student;
select * from course;
-- 두개 테이블를 카디넬리곱으로 모두 조인하기 
select * from student, course;
-- cou_id가 동일한 것을 조건으로 조인하기
...
select count(*) 모바일수강인원 from student s, course c 
      where s.cou_id = c.cou_id and c.cou_name = '모바일';      
-- 남학생이고 성윤정 선생님 과목을 수강하는 학생의 학번과, 나이, 이메일, 수강코드, 수강명을 출력하기
...
-- student테이블에 컬럼 추가하기 memto_id number(4)
  desc student;
  alter table student
     add memto_id number(4);
-- memto_id 컬럼 값 추가하기 103, 105, 111, 101,103,102,102,106,105,105,104,104,110,110,112,112
   select * from student;
...
select member.ename 사원이름, manager.ename 직속상관이름
   ...
   where member.mgr = manager.empno(+); -- left outer join 동일 왼쪽에 있는 테이블은 모두 나오기, 오른쪽 테이블의 null 
   
select member.ename 사원이름, manager.ename 직속상관이름
   from emp member,  emp manager
   where member.mgr(+) = manager.empno; -- right outer join 동일 오른쪽에 있는 테이블은 모두 나오기, 왼쪽 테이블의 null 

select member.ename 사원이름, manager.ename 직속상관이름
   from emp member,  emp manager;
 --  where member.mgr(+) = manager.empno(+); -- 지원안함 대신 where 빼면 나옴 
 -- full outer join 동일 오른쪽에 있는 테이블은 모두 나오기, 왼쪽 테이블의 모두다 나오기 
...
select  s2.stu_name from student s1 right outer join student s2  
...
-- 사원이 없는 부서명을 출력하세요.
select d.dname from emp e right outer join dept d on e.deptno = d.deptno where e.deptno is null;
