...
const ageArr3 = ageArr1.map(
  (age, index) => index + 1 + "번째 " + (age < 20 ? "미성년" : "성년")
);
console.log(ageArr3);
...
const evenArr1 = intArr1.filter((n) => {
  return n % 2 === 0;
...
//원하는 것은 군대 대상자를 배열로 만들기
const 입대대상자 = personName
  .filter((name, index) => gender[index])
  .filter((name, index) => age8[index] >= 20);
console.log(입대대상자);
console.log(
  `${입대대상자}은 신검대상자 입니다. 가까운 병무청을 방문하여주세요.`
);

const schoolName = ["서울대", "연세대", "고려대"];
const level = [1, 2, 2];
//이순신이 2등급으로 고등학교 졸업했다. 갈 수 있는 대학은?
const student = {
  name: "이순신",
  level: 2,
  graduate: true, //졸업여부
};
const 입학가능학교 = schoolName.filter(
  (schoolName, index) => level[index] >= student.level
);
console.log(입학가능학교);
console.log(`${student.name}씨가 갈 수 있는 대학은 ${입학가능학교}입니다.`);
